...
// Bounded LRU Blob URL Cache (Prevents browser memory leaks)
...
// --- 1. Mode Initialization & Cloud Banner Visibility ---
function updateCloudScannerVisibility() {
  if (IS_LOCAL_HOST) {
    if (cloudScannerCard) cloudScannerCard.classList.add("hidden");
    if (localHostBanner) localHostBanner.classList.remove("hidden");
    return;
  }

  if (localHostBanner) localHostBanner.classList.add("hidden");
  if (cloudScannerCard) {
    if (hasIndexedDbLoaded) {
      cloudScannerCard.classList.add("hidden");
    } else {
      cloudScannerCard.classList.remove("hidden");
    }
  }
}
...
function openCompletionModal(type) {
  modalTag.className = "tag-experimental-inline";
  modalTag.style.background = "rgba(35, 165, 90, 0.15)";
  modalTag.style.color = "#86efac";
  modalTag.style.borderColor = "rgba(35, 165, 90, 0.3)";

  if (type === "download") {
    modalTag.textContent = "Download Complete";
    modalTitle.textContent = "Your Attachments Are Ready!";
    modalBody.innerHTML = `
      <p>
    ...
    modalTag.textContent = "Indexing Complete";
    modalTitle.textContent = "Demo Labels & OCR Ready!";
    modalBody.innerHTML = `
      <p>
        Successfully tagged your first 100 images and saved <code>demo.db</code> directly to your <code>disdump-download</code> folder.
      </p>
      <p>
        To index your entire library privately with no limits, run <code>python qwen-rtx.py</code> locally on your GPU.
      </p>
      <p class="subtle-note">
        I fund these demo GPU hours out-of-pocket. If you found this useful, consider tipping on Ko-fi or joining the Discord:
      </p>
      <div class="support-action-row">
        <a href="https://github.com/wyOmar/DisDump" target="_blank" rel="noopener noreferrer" class="btn btn-secondary btn-modal-action">
          <svg class="btn-icon" viewBox="0 0 24 24" fill="currentColor">
...
// --- 8. Open Existing Folder Workflow (Smooth Live Progress) ---
async function openExistingFolder() {
  try {
    ...
    for (let d = 0; d < dirEntries.length; d++) {
        ...
        for await (const fileHandle of handle.values()) {
          if (fileHandle.kind === "file") {
            ...
            if (scannedFilesCount % 150 === 0) {
              ...
              dlProgressBar.style.width = `${approxProgress}%`;
              dlStatusDetail.textContent = `Reading /${folderName} • Found ${formatNumber(scannedFilesCount)} files...`;
              renderBadgeUI();
            }
          }
        }
      ...
      dlProgressBar.style.width = `${dirProgress}%`;
      dlStatusDetail.textContent = `Completed /${folderName} • Found ${formatNumber(scannedFilesCount)} files...`;
    }
